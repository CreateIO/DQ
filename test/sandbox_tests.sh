...
do_curl "${dq_host}/DQ/template?resource=tabs-" tabs_generic_novers \
'{"tabs":[{"name":"Property","tabs":[{"name":"Overview",'

do_curl "${dq_host}/DQ/template?resource=tabs-&version=1.0.0" tabs_generic \
'{"tabs":[{"name":"Property","tabs":[{"name":"Overview",'

do_curl "${dq_host}/DQ/template?resource=tabs-&version=1.0.0&branch=master" tabs_branch \
'{"tabs":[{"name":"Property","tabs":[{"name":"Overview",'
...
do_curl "${dq_host}/DQ/docURL/?version=1.0.0" docurl_generic \
'[{"fileNames":[],"status":"success","count":0}]'

do_curl "${dq_host}/DQ/docURL?propertyID=102613&version=1.0.0" docurl_single \
'[{"id":1449,"assets":"Spring_Valley","fileNames":[],"status":"success","count":0}]'

do_curl "${dq_host}/DQ/docCollection?propertyIdBin=153058&propertyIdBin=5473&version=1.0.0&type=pdf" doccollection \
'[{"id":346,"property_id":"5473","assets":"1010_Massachusetts_Avenue","fileNames":[],"status":"success","count":0}]'

do_curl "${dq_host}/DQ/docURL?propertyID=131292&version=1.0.0" docurl_double \
'[{"id":1482,"assets":"3365_14th_Street","fileNames":[],"status":"success","count":0},{"id":47,'

do_curl "${dq_host}/DQ/docURL?parcelID=3434&version=1.0.0" docurl_single_parcelid \
'[{"fileNames":[],"status":"success","count":0}]'

do_curl "${dq_host}/DQ/regionAsset?region=US11001&resource=fredRecessionDates" regionasset \
'[{"name":"polygon 1","points":[{"date":"8/1/1929","y":0},{"date":"8/1/1929","y":1},'

do_curl "${dq_host}/DQ/regionAsset?region=US11001_50000&resource=fredRecessionDates" regionasset3 \
'[{"name":"polygon 1","points":[{"date":"8/1/1929","y":0},{"date":'

do_curl "${dq_host}/DQ/nearbyregions?regionID=US11001" nearbyRegions \
'[{"region_id":"US24033","region_full_name":"Prince George'

do_curl "${dq_host}/DQ/region?lat=38.9041485&long=-77.017094" fetchRegion \
'[{"region_id":"US11","region_full_name":"District of Columbia, US","region_level":1},{"region_id":"US",'

do_curl "${dq_host}/DQ/regiondata?regionID=US11001" regiondata \
'[{"region_level":2,"region_typename":null,"region_child_typename":"","region_full_name":"Washington, DC, US"'

do_curl "${dq_host}/DQ/version" fetchversion \
...
'[{"source":"OCTO","id":897,"field_name":["airRights","property.airRights","core.airRights"]'
...
do_curl "${dq_host}/DQ/regionFind?name=Washington" regionname2 \
