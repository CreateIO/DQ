...
do_curl "${dq_proto}://${dq_host}/DQ/template?resource=tabs-&version=1.0.0&branch=master&passphrase=test-Access*98765!" test_tabs_branch.json | tee curl_result.txt
if !(grep -q '^200' curl_result.txt); then
    echo "Test failed: did not receive '200' status"
fi
...
if !(grep -Fq '[{"region_id":"US01129","region_full_name":"Washington County, AL, US","region_level":2}' test_regionname2.json); then
